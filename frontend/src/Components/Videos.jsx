...
const Videos = ({ videopost }) => {
  return (
    <SkeletonTheme baseColor="#e0e0e0" highlightColor="#f5f5f5">
      <div className="px-5 pb-6 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6">
        {videopost.length > 0
          ? videopost.map((data) => (
              <Link
                to={`/post/${data?._id || data?.id}`}
              ...
              >
                ...
                >
                  <CardActionArea sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
                    {data?.video ? (
                      <CardMedia
                        sx={{ height: 180, borderTopLeftRadius: "12px", borderTopRightRadius: "12px" }}
                        component="video"
                        src={data?.video}
                        controls
                      />
                    ) : (
                      <Skeleton height={180} width="100%" borderRadius="12px 12px 0 0" />
                    )}
